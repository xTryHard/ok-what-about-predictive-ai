...
/**
 * DJL Translator that handles YOLOv11 pre-processing (letterbox) and
 * post-processing (multi-class confidence filtering + NMS) for the baseball detection model.
 *
 * Output shape: (1, 8, 8400) — 4 box coords + 4 class scores per anchor.
 */
class BaseballDetectorTranslator implements NoBatchifyTranslator<Image, DetectedObjects> {

    private static final List<String> CLASSES = Arrays.asList("Homeplate", "Baseball", "Rubber", "background");
    ...
    public NDList processInput(TranslatorContext ctx, Image input) {
        ...
        // Scale image preserving aspect ratio, then center it on a gray 640x640 canvas
    ...
    public DetectedObjects processOutput(TranslatorContext ctx, NDList list) {
        ...
        int numAnchors = 8400; // YOLOv11 default for 640x640 input
        int numClasses = CLASSES.size(); // 4
        ...
        for (int i = 0; i < numAnchors; i++) {
            // Find the class with the highest score (rows 4..7)
            int bestClass = -1;
            float bestScore = -1f;
            for (int c = 0; c < numClasses; c++) {
                float classScore = flatData[(4 + c) * numAnchors + i];
            ...
            if (CLASSES.get(bestClass).equals("background")) continue;
            ...
            // Map from 640x640 letterbox space -> original image space (normalized 0-1)
            ...
            scores.add((double) bestScore);
            classIndices.add(bestClass);
        }

        return applyNMS(candidates, scores, classIndices);
    }

    private DetectedObjects applyNMS(List<Rectangle> boxes, List<Double> scores, List<Integer> classIndices) {
